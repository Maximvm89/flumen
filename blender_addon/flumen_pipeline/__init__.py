...
def register():
    ...
    bpy.types.WindowManager.flumen_apply_modifiers = bpy.props.BoolProperty(
        name="Apply modifiers in publish", default=False,
    ...
    bpy.types.WindowManager.flumen_force_publish = bpy.props.BoolProperty(
        ...
        description="Publish a new shot version even when no animation changed "
                    "since the last one (e.g. camera/layout tweaks, or to re-render "
                    "the playblast)")
    ...
    if os.environ.get("FLUMEN_CACHE_SHOT"):
        # Headless 'Cache shot': the actual build+bake+publish runs from
        # flumen/blender_cache_shot.py, a second --python AFTER this bootstrap
        # (bpy.data is restricted during register(), and app.timers never fire
        # in -b mode). Here we only skip the interactive startup hooks.
        return
