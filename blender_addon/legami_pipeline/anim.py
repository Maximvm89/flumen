...
def build_anim_manifest(version: int, element_actions: dict) -> dict:
    """The <base>_vNNN_anim.manifest.json payload: which action sits on which object,
    grouped per element. `element_actions` is {element_id: {object_name: action_name}}
    (empty element maps dropped)."""
    elements = {eid: dict(objs) for eid, objs in (element_actions or {}).items()
                if objs}
    return {"version": int(version), "elements": elements}
